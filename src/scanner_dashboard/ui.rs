...
use crate::scanner_dashboard::state::{ScannerDashboardState, ScanStatus, ViewMode};
...
fn render_header(f: &mut Frame, state: &ScannerDashboardState, area: Rect) {
    ...
    let depth_str = state.scan_depth
...
fn render_scan_progress(f: &mut Frame, state: &ScannerDashboardState, area: Rect) {
    ...
    let progress_info = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("Libraries Found: ", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
    ...
    let progress_widget = Paragraph::new(all_lines)
...
fn render_recent_discoveries(f: &mut Frame, state: &ScannerDashboardState, area: Rect) {
    let items: Vec<ListItem> = state.recent_libraries
        .iter()
        .rev() // Newest first
        .map(|lib| {
            ...
            let version_str = lib.version.as_ref()
                .map(|v| format!(" ({})", v))
                .unwrap_or_default();
            ...
            let lines = vec![
                ...
                Line::from(vec![
                    Span::raw("   "),
                    Span::styled(lib.path.display().to_string(), Style::default().fg(Color::Gray)),
                ]),
                Line::from(vec![
                    Span::raw("   Category: "),
    ...
    let list = List::new(items)
        .block(Block::default().title(" Recently Discovered ").borders(Borders::ALL));
...
fn render_category_view(f: &mut Frame, state: &ScannerDashboardState, area: Rect) {
    ...
    for (category, libs) in categories_data {
        ...
        if is_expanded {
            for lib in libs.iter().take(20) { // Limit to 20 per category for display
                let risk_color = match lib.risk_level {
                ...
                let vendor_str = lib.vendor.as_ref()
                    .map(|v| format!(" [{}]", v))
                    .unwrap_or_default();
    ...
    let title = if state.filter.is_active() {
        format!(" Category Breakdown (Filtered: {}/{}) ",
...
fn render_distribution_stats(f: &mut Frame, state: &ScannerDashboardState, area: Rect) {
    ...
    let category_data: Vec<(&str, usize, Color)> = vec![
        ("SSL/TLS", state.category_counts.get(&LibraryCategory::SslTls).copied().unwrap_or(0), Color::LightBlue),
        ("General Crypto", state.category_counts.get(&LibraryCategory::GeneralCrypto).copied().unwrap_or(0), Color::Magenta),
        ("Post-Quantum", state.category_counts.get(&LibraryCategory::PostQuantum).copied().unwrap_or(0), Color::Green),
        ("Hash Functions", state.category_counts.get(&LibraryCategory::HashFunction).copied().unwrap_or(0), Color::Cyan),
        ("Rust Crypto", state.category_counts.get(&LibraryCategory::RustCrypto).copied().unwrap_or(0), Color::LightYellow),
        ("Node.js Crypto", state.category_counts.get(&LibraryCategory::NodeCrypto).copied().unwrap_or(0), Color::Yellow),
        ("Python Crypto", state.category_counts.get(&LibraryCategory::PythonCrypto).copied().unwrap_or(0), Color::LightGreen),
        ("Other", state.category_counts.get(&LibraryCategory::Other).copied().unwrap_or(0), Color::Gray),
    ];

    let chart = create_bar_chart(category_data, 12);
    category_lines.extend(chart);

    let category_widget = Paragraph::new(category_lines)
    ...
    let risk_data: Vec<(&str, usize, Color)> = vec![
        ("Critical Risk", state.risk_counts.get(&RiskLevel::Critical).copied().unwrap_or(0), Color::Red),
        ("High Risk", state.risk_counts.get(&RiskLevel::High).copied().unwrap_or(0), Color::LightRed),
        ("Medium Risk", state.risk_counts.get(&RiskLevel::Medium).copied().unwrap_or(0), Color::Yellow),
        ("Low Risk", state.risk_counts.get(&RiskLevel::Low).copied().unwrap_or(0), Color::Cyan),
        ("PQ-Ready", state.risk_counts.get(&RiskLevel::None).copied().unwrap_or(0), Color::Green),
    ];

    let risk_chart = create_bar_chart(risk_data, 12);
    risk_lines.extend(risk_chart);

    let risk_widget = Paragraph::new(risk_lines)
        .block(Block::default().borders(Borders::ALL));
...
fn render_summary_stats(f: &mut Frame, state: &ScannerDashboardState, area: Rect) {
    use crate::scanner_dashboard::widgets::{format_size, format_duration};
    ...
    let mut summary_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{}", state.library_count()), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
            Span::styled("Total Size:          ", Style::default().fg(Color::Gray)),
            Span::styled(format_size(state.total_size), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Unique Vendors:      ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", state.unique_vendors.len()), Style::default().fg(Color::White)),
        ]),
        Line::from(""),
        Line::from(Span::styled(
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{}", state.scan_progress.files_scanned), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Directories:         ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", state.scan_progress.dirs_scanned), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Scan Rate:           ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} files/sec", state.scan_progress.scan_rate), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Runtime:             ", Style::default().fg(Color::Gray)),
            Span::styled(format_duration(runtime.as_secs()), Style::default().fg(Color::White)),
        ]),
    ];

    // Add final scan speed metric (only when scan is complete)
    if matches!(state.scan_status, crate::scanner_dashboard::state::ScanStatus::Completed) {
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                match state.scan_status {
                    crate::scanner_dashboard::state::ScanStatus::Running => Style::default().fg(Color::Green),
                    crate::scanner_dashboard::state::ScanStatus::Paused => Style::default().fg(Color::Yellow),
                    crate::scanner_dashboard::state::ScanStatus::Completed => Style::default().fg(Color::Blue),
                    crate::scanner_dashboard::state::ScanStatus::Error(_) => Style::default().fg(Color::Red),
                },
            ),
        ]),
    ]);
    ...
    let summary_widget = Paragraph::new(summary_lines)
        .block(Block::default().title(" Statistics & Analysis ").borders(Borders::ALL));
...
fn render_migration_view(f: &mut Frame, state: &ScannerDashboardState, area: Rect) {
    ...
    if state.migration_recommendations.is_empty() {
        let prompt = Paragraph::new(vec![
            ...
            Line::from(""),
            Line::from(Span::raw("  Press 'g' to generate migration recommendations")),
            Line::from(""),
            Line::from(Span::styled(
        ...
        .block(Block::default().title(" Post-Quantum Migration Planner ").borders(Borders::ALL));
    ...
    render_migration_summary(f, state, total_vulnerable, not_started, in_progress, completed, chunks[0]);
    ...
    for (priority, libs) in priorities_data {
        ...
        items.push(ListItem::new(vec![Line::from(""), header_line, Line::from("")]));
        ...
        for lib in libs.iter().take(10) {
            if let Some(rec) = state.migration_recommendations.get(&lib.name) {
                let status = state.migration_statuses.get(&lib.name)
                ...
                let lib_header = Line::from(vec![
                    ...
                    Span::raw(" "),
                    Span::styled(&lib.name, Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
                    Span::raw("  "),
                    Span::styled(
                        format!("[{}]", status.label()),
                ...
                let recommended = Line::from(vec![
                    Span::raw("     Migration:   "),
                    Span::styled(&rec.recommended_algorithm, Style::default().fg(Color::LightGreen)),
                ]);

                // Strategy and timeline
                let strategy_label = match rec.migration_strategy {
                    crate::scanner_dashboard::state::MigrationStrategy::DirectReplacement => "Direct Replacement",
                    crate::scanner_dashboard::state::MigrationStrategy::HybridMode => "Hybrid Classical+PQ",
                    crate::scanner_dashboard::state::MigrationStrategy::PurePostQuantum => "Pure Post-Quantum",
                    crate::scanner_dashboard::state::MigrationStrategy::NoMigrationNeeded => "No Migration Needed",
                };
        ...
        if libs.len() > 10 {
            items.push(ListItem::new(vec![
                Line::from(vec![
                    Span::raw("     "),
                    Span::styled(
                        format!("... and {} more libraries in this priority", libs.len() - 10),
                        Style::default().fg(Color::DarkGray),
                    ),
                ]),
                Line::from(""),
...
fn render_migration_summary(
    ...
    let summary_lines = vec![
        Line::from(vec![
            Span::styled("  Libraries Needing Migration: ", Style::default().fg(Color::Gray)),
            Span::styled(
            ...
            Span::styled(format!("{}", not_started), Style::default().fg(Color::White)),
            Span::raw("  |  "),
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(if completion_pct > 50.0 { Color::Green } else { Color::Yellow }),
            ),
        ]),
    ];

    let summary = Paragraph::new(summary_lines)
        .block(Block::default().title(" Migration Overview ").borders(Borders::ALL));
...
fn render_footer(f: &mut Frame, state: &ScannerDashboardState, area: Rect) {
    ...
    let view_controls = match state.current_view {
        ViewMode::LiveScan => " q:Quit h:Help Space:Pause Tab:Views d:Details e:JSON v:CSV f:Filter s:Sort ",
        ViewMode::Category => " q:Quit h:Help Tab:Views d:Details e:JSON v:CSV a:Expand c:Collapse ",
    ...
    let filter_status = if state.filter.is_active() {
        format!(" [Filter Active: {}/{}] ", state.get_filtered_count(), state.library_count())
    } else {
        String::new()
    };
...
fn render_detail_view(f: &mut Frame, state: &ScannerDashboardState) {
    ...
    let mut detail_lines = vec![
        Line::from(vec![
            Span::styled("Library Details", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(vec![
    ...
    detail_lines.push(Line::from(vec![
        Span::styled("Risk Assessment", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
    ]));
    detail_lines.push(Line::from(""));
    detail_lines.push(Line::from(vec![
    ...
    detail_lines.push(Line::from(vec![
        ...
        Span::styled(
            ...
            Style::default().fg(if lib.quantum_vulnerable { Color::LightRed } else { Color::Green }),
        ),
    ]));
    ...
    if let Some(rec) = state.migration_recommendations.get(&lib.name) {
        detail_lines.push(Line::from(""));
        detail_lines.push(Line::from(vec![
            Span::styled("Migration Recommendation", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
        ]));
        detail_lines.push(Line::from(""));
        detail_lines.push(Line::from(vec![
        ...
        detail_lines.push(Line::from(vec![
            ...
            Span::styled(format!("{} months", rec.timeline_months), Style::default().fg(Color::Cyan)),
        ]));
        detail_lines.push(Line::from(vec![
            ...
            detail_lines.push(Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(match status {
                        ...
                        crate::scanner_dashboard::state::MigrationStatus::InProgress => Color::Yellow,
    ...
    if let Some(deps) = state.dependency_map.get(&lib.name) {
        if !deps.is_empty() {
            detail_lines.push(Line::from(""));
            detail_lines.push(Line::from(vec![
                Span::styled("Dependencies", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            ]));
            detail_lines.push(Line::from(""));
            detail_lines.push(Line::from(vec![
                Span::styled("Used by ", Style::default().fg(Color::Gray)),
                Span::styled(format!("{} binaries:", deps.len()), Style::default().fg(Color::White)),
            ]));
...
fn render_completion_notification(f: &mut Frame, state: &ScannerDashboardState) {
    ...
    let notification_lines = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("  Total Libraries Found:      ", Style::default().fg(Color::Gray)),
            Span::styled(
        ...
        Line::from(vec![
            Span::styled("  Post-Quantum Ready:         ", Style::default().fg(Color::Gray)),
            Span::styled(
                format!("{} ({:.1}%)",
                    pq_ready,
                    if total_libraries > 0 {
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("EXPORT OPTIONS", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::styled("scan_results_YYYYMMDD_HHMMSS.json", Style::default().fg(Color::DarkGray)),
        ]),
        Line::from(vec![
            Span::styled("  Press ", Style::default().fg(Color::Gray)),
            Span::styled("'v'", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            Span::styled(" to export library inventory to ", Style::default().fg(Color::Gray)),
            Span::styled("CSV", Style::default().fg(Color::Cyan)),
        ]),
        Line::from(vec![
            Span::styled("      → ", Style::default().fg(Color::DarkGray)),
            Span::styled("library_inventory_YYYYMMDD_HHMMSS.csv", Style::default().fg(Color::DarkGray)),
        ]),
        Line::from(vec![
            Span::styled("  Press ", Style::default().fg(Color::Gray)),
            Span::styled("'4'", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            Span::styled(" to view Migration Planner", Style::default().fg(Color::Gray)),
        ]),
        Line::from(vec![
            Span::styled("      (then press ", Style::default().fg(Color::DarkGray)),
            Span::styled("'e'", Style::default().fg(Color::DarkGray)),
            Span::styled(" for migration_plan_YYYYMMDD_HHMMSS.md)", Style::default().fg(Color::DarkGray)),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::styled("  Files saved to current directory: ", Style::default().fg(Color::Gray)),
            Span::styled(
                std::env::current_dir()
...
fn render_help_overlay(f: &mut Frame, _state: &ScannerDashboardState) {
    ...
    let help_lines = vec![
        Line::from(vec![
            Span::styled("Keyboard Shortcuts Reference", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),

        // Global Controls
        Line::from(vec![
            Span::styled("GLOBAL CONTROLS", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(vec![
        ...
        // View-Specific
        Line::from(vec![
            Span::styled("VIEW-SPECIFIC CONTROLS", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::styled("  Live Scan & Category Views:", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("  Category View Only:", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::styled("  Migration Planner View:", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            Span::raw("  • Generated files are saved with timestamps in the current directory"),
        ]),
    ];
